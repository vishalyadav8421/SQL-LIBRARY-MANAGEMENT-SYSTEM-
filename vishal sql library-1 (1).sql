...
show databases;
use lms;
Create table author(author_id int (11) primary key, author_first_name varchar(50), author_last_name varchar(50));
desc author;
insert into author(author_id, author_first_name, author_last_name) 
values(1, 'William Dean', 'Howells'), (2, 'Frederic', 'Brown'), (3, 'Jack', 'London'), (4, 'Albert', 'Blaisdell'), (5, 'Ellis', 'Butler'),
...
(14, 'George', 'Orwell'),
(15, 'Publius', 'Ovid'),
(16, 'Robert Louis', 'Stevenson'), (17, 'Virginia', 'Woolf'),
(18, 'George', 'Eliot'),
(19, 'Amelia B.', 'Edwards'),
(20, 'Fyodor', 'Dostoevsky'),
(21, 'Emily', 'Dickinson'),
(22, 'Edna', 'Ferber'),
(23, 'Joseph Sheridan', 'LeFanu'), (24, 'John', 'DosPassos'),
(25, 'Ruth', 'Stuart'), (26, 'Vladimir', 'Nabokov'),
(27, 'Johanna', 'Spyri'),
(28, 'Ernest', 'Dowson'),
...
create table type(type_id int(11), type varchar(50));
desc type;
insert into type(type_id, type) values(1, 'Science fiction'),
(2, 'Satire'),
(3, 'Drama'),
(4, 'Action and Adventure'), (5, 'Romance'),
(6, 'Mystery'),
(7, 'Horror'),
(8, 'Health'),
(9, 'Guide'),
...
create table book(bkid int(11) primary key not null, title varchar(50), page_count int(11), author_id int(11), type_id int(11));
desc book;
insert into book(bkid, title, page_count, author_id, type_id) 
values (1, 'A Daughter of the Snows', 199, 3, 9),
...
create table customer(custID int(11), fname varchar(30), lname varchar(30), gender varchar(5), age int(3), sub_start_date  date, sub_end_date date);
desc customer;
insert into customer(custID, fname, lname, gender, age, sub_start_date, sub_end_date) values(1, 'Hazel', 'Green', 'F', 28, '2020-05-15', '2023-05-15'),
...
create table transaction(trans_id int(11) primary key, cust_id int(11), bkID int(11), issue_date datetime, return_date datetime );
desc transaction;
insert into transaction(trans_id, cust_id, bkID, issue_date, return_date) 
values (1, 37, 24, '2021-08-31 04:30:00', '2021-09-17 00:15:00'),
...
(60, 15, 12, '2022-02-27 15:38:00', '2022-03-11 14:44:00');
 ...
 /* to view all above created tables*/
 ...
 select*from customer;
 select*from transaction;
 select customer.fname, customer.custID, transaction.bkID 
 from customer 
 left join transaction 
 on customer.custID=transaction.cust_id;
select customer.custID, concat(customer.fname," " ,customer.lname) as customer_full_name
 from customer 
 left join transaction 
 on customer.custID=transaction.cust_id where bkID is null;
 ...
 select bkid, title, page_count from book where page_count = (select max(page_count) from book);
...
concat(customer.fname," " ,customer.lname) as customer_full_name,
MAX(DATEDIFF(Transaction .return_date, Transaction .issue_date)) AS MaxBorrowedDays
from customer
join transaction
on customer.custID=transaction.cust_id
group by customer.custID
order by MaxBorrowedDays DESC
LIMIT 1;


 /*4.display list of all books along with their authors*/
 
select*from author;
select*from book;

select book.bkid, book.title, concat(author.author_first_name," ", author.author_last_name) as author
from book 
join author
on author.author_id= book.author_id;


/*5.Display list of top 20 most borrowed books*/

select*from book;
select*from transaction;

select transaction.bkID, book.title, count(transaction.trans_id) as times_borrowed 
from book 
join transaction
on transaction.bkID=book.bkid
GROUP BY book.title
ORDER BY times_borrowed  desc
limit 20 ;

/*6.Display list of top 10 customers who have borrowed most book*/

select*from transaction;
select*from customer;

select  customer.custID, concat( customer.fname," ",customer.lname) as name,  count(transaction.trans_id) as book_borrowed
from customer
join transaction
on transaction.cust_id=customer.custID
